...
def add_mongo_proxy(db,proxy_record):
	#id = hashlib.md5(host + ":" + port).hexdigest()
	#proxy_record = {'_id':id, 'proxy':proxy}
	try:
    		db.proxies.insert(proxy_record)
	except pymongo.errors.DuplicateKeyError, e:
    		print "Duplicate Found!\n"
    
def get_mongo_proxy_all(db):
    return db.proxies.find()
...
def queueThread():
	...
	while not workerQueue.empty():
		...
		db = get_mongo_db()
		id = hashlib.md5(item).hexdigest()
		proxy_record = {'_id':id, 'proxy':item}
		add_mongo_proxy(db,proxy_record)
